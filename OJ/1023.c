...
/********************************************************************************************
    ...
    输入描述
    Input
    输入文件apple.in包括两行数据。第一行包含10个100到200之间（包括100和200）的整数（以厘米为单位）
...
int main(){
    int appleHeight[10] = {0};
    int manHeight = 0, sumHeight = 0;
    int i = 0, count = 0;
    ...
    sumHeight = manHeight + 30;

    for(i = 0; i < 10; i++){
        if(sumHeight >= appleHeight[i]){
            count++;
        }
    }
    printf("%d\n", count);
